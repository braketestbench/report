\section{Speed Reference Output Channel}
	...
	\subsection{Filter characteristics definition}\label{ssec:filterCharacteristicsDefinition}
		...
		\begin{figure}[htbp]
			\centering
				\includegraphics[scale=0.25]{figuras/fig-fiveToTenVolts}
	...
	\subsection{Sallen-Key Low Pass Active Filter}
		...
		Based on all the previously calculations and on \cite{texas1999sallenkey}, the final circuit will be the one in Figure \ref{fig:pwmToSpeedReferenceCircuit}. A voltage follower buffer was added at the output to prevent the components from output protection circuit to affect the filter perfomance. The protection circuit was composed of a TVS diode and series resistors. A TVS diode is a Transient Voltage Supression Diode, also called thyrector, is an electronic component used to protect electronics from voltage spikes induced on connected wires \cite{lepkowski2006evaluating}. It works like any other diode, the difference regards the surge current this devices can work during clamping without failing. The series resistors are used to protect the OPAMP output and to reduce the current induced to the TVS.
		\begin{figure}[htbp]
			\centering
				\includegraphics[scale=0.25]{figuras/fig-pwmToSpeedReferenceCircuit}
